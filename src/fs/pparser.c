...
static int pathparser_get_drive_by_path(const char **path){
    ...
    // add three bytes to skip drive number e.g 0:/ 1:/
    *path += 3;
    return drive_no;
}
...
static const char * pathparser_get_path_part(const char **path){
    char *result_path_part = kzalloc(sizeof(PEACHOS_MAX_PATH));
    int i = 0;
    ...
    if(i == 0){
        // we didn't parse anything so free the pointer passed to us
