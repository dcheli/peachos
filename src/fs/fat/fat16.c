...
} __attribute__((packed));  // assures the compiler doesn't rearrange the variables in the structure
                            // also, it has to be packed because we are reading and writing this to the disk
struct fat_header
{
    uint8_t short_jmp_ins[3];   // see boot.asm jmp short star, that will be loaded into this array
    uint8_t oem_identifier[8];
    uint16_t bytes_per_sector;
    uint8_t sectors_per_cluster;
    uint16_t reserved_sectors;  // number of sectors befor the first FAT
    uint8_t fat_copies;
...
struct fat_private
{
    ...
    // setup your data streamers
    // Used to stream data clusers
...
int fat16_get_total_items_for_directory(struct disk *disk, uint32_t directory_start_sector){
    ...
    struct fat_directory_item item;
    struct fat_directory_item empty_item;       // created on stack and uninitialzied
    memset(&empty_item, 0, sizeof(empty_item)); // initialize it
...
// convert a sector number into an absolute number on the disk
...
int fat16_get_root_directory(struct disk *disk, struct fat_private *fat_private, struct fat_directory *directory) {
    ...
    if(root_dir_size % disk->sector_size){  
        total_sectors += 1;                 // accomdates for values larger than a sector (adds an additional one)
    }
    ...
    if(diskstreamer_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != PEACHOS_ALL_OK){
        // we failed to seek to the part of the disk that we need to read from
        res = -EIO;
        goto out;
    }

    // now we can read the root directory
...
int fat16_resolve(struct disk *disk){
    ...
    // returning a 0 confirms that we can manage this disk
    int res = 0;
    struct fat_private *fat_private = kzalloc(sizeof(struct fat_private));
    fat16_init_private(disk, fat_private);

    // we are binding the disk
    ...
    // read disk bytes into the header
    ...
    // check if you can load the root directory
...
void fat16_to_proper_string(char **out, const char *in){
    ...
    while(*in != 0x00 && *in != 0x20){ // look for null terminator or space
        **out = *in;
        *out += 1;
        in += 1;
    }
    if(*in == 0x20) { // if the character is a space
        **out = 0x00;
    }
}
...
void fat16_get_full_relative_filename(struct fat_directory_item *item, char *out, int max_len){
    ...
    if(item->ext[0] != 0x00 && item->ext[20]){
        *out_tmp++ = '.';
...
static uint32_t fat16_get_first_cluster(struct fat_directory_item *item){
    return (item->high_16_bits_first_cluster) | item->low_16_bits_first_cluster; // joins the high and low bits
}
...
static int fat16_get_cluster_for_offset(struct disk *disk, int starting_cluster, int offset){
    // this will get the correct cluster to use based on the starting_cluster and the offset
    int res = 0;
    struct fat_private *private = disk->fs_private;
    ...
    for(int i = 0; i < clusters_ahead; i++){
        ...
        if(entry == 0xFF8 || entry == 0xFFF){
            // we are at the last entry in the subdirectory or file
            res = -EIO;
            goto out;
        }

        // check is sector is marked as bad
        if(entry == PEACHOS_FAT16_BAD_SECTOR){
            res = -EIO;
            goto out;
        }
        // Check of reserved sectors
...
static int fat16_read_internal_from_stream(struct disk *disk, struct disk_stream *stream, int cluster, int offset, int total, void *out){
    ...
    total -= total_to_read;
    if(total > 0){ // we have more to read
...
// this loads the entire FAT directory
struct fat_directory* fat16_load_fat_directory(struct disk *disk, struct fat_directory_item *item){
    int res = 0;
    struct fat_directory *directory = 0;

    struct fat_private *fat_private = disk->fs_private;
    if(!(item->attribute & FAT_FILE_SUBDIRECTORY)){    // we only want directories, not files
        res = -EINVARG;
        goto out;
    }
...
struct fat_item* fat16_find_item_in_directory(struct disk *disk, struct fat_directory *directory, const char *name){
    ...
    for(int i = 0; i < directory->total; i++){
        // this will return the filename and its extension
...
struct fat_item* fat16_get_directory_entry(struct disk *disk, struct path_part *path){
    ...
    // this will look for an item (e.g. /test.txt or /abc if /abc/test.txt)in the passed in directory
...
int fat16_read(struct disk* disk, void *descriptor, uint32_t size, uint32_t nmemb, char *out_ptr){
    // returns the amout of nmemb's read
    int res = 0;
