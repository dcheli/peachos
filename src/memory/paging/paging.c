...
// it's static so it's not accessible outside of this file
static uint32_t *current_directory = 0; 

struct paging_4gb_chunk *paging_new_4gb(uint8_t flags){
    
    // This creates the page directory and the page tables/entries
    // creates a linear memory layout e.g. 0x400000 -> 0x400000
...
// this resolves a virtual address to a directory entry and table index
...
// count is how many pages we want to map
...
// This will set a page table entry to the value provided
int paging_set(uint32_t *directory, void *virt, uint32_t val){
   ...
   if(!paging_is_aligned(virt)){
    ...
    uint32_t *table = (uint32_t*)(entry & 0xfffff000); // this will just extract the address; not the flags
