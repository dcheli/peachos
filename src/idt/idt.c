...
void idt_init() {
    memset(idt_descriptors, 0, sizeof(idt_descriptors));    // This will create a bunch of null descriptors
...
void* isr80_handle_command(int command, struct interrupt_frame *frame){
    void *result = 0;
...
void *isr80h_handler(int command, struct interrupt_frame *frame){
    ...
    res = isr80_handle_command(command, frame);
    task_page();
    return res;
}